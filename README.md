...
- Runtime script that calls the AI model in this repository: `index.html` (client-side summarization flow; functions `generateAndSaveSummaries()` and `askMistral()`).
- Note: `mistral_summarizer.py` and `src/mistralClient.js` are not present in this repository version.
